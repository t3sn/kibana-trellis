...
var module = require('ui/modules').get('bar-vis-trellis');
...
module.controller('TrellisController', function($scope, Private) {

	var filterManager = Private(require('ui/filter_manager'));
	//Private(require('plugins/bar-vis-trellis/ext/grouped-categories'));
	
	$scope.filter = function(tag) {
		// Add a new filter via the filter manager
		filterManager.add(
			// The field to filter for, we can get it from the config
			$scope.vis.aggs.bySchemaName['bucket'][0].params.field,
	...
	$scope.$watch('esResponse', function(resp) {
		...
		// Retrieve the id of the configured bucket aggregation
		var bucketAggId = $scope.vis.aggs.bySchemaName['bucket'][0].id;
		
		// Get the buckets of that aggregation
		var buckets = resp.aggregations[bucketAggId].buckets;

		//**********************************************
		//console.log('HighCharts :' + Highcharts);
		//console.log('jQuery : ' + $);

		var charts = [], $containers = $('#trellis td');
		var metricsAggArr = $scope.vis.aggs.bySchemaName['metric'];
		
		$scope.metrics = {};
		$scope.categories = buckets.map(function(bucket){
			return bucket.key;
		});	
		console.log($scope.categories);

		//var value = resp.aggregations && resp.aggregations[category] && resp.aggregations[category].value || resp.hits.total;		
		var datasets = metricsAggArr.map(function(metric) {
			var colarr = ['#102d4c', '#27425d', '#3f566f', '#576c81', '#6f8193', '#8796a5', '#9fabb7', '#b7c0c9', '#cfd5db', '#e7eaed'];
			var data = [];
			buckets.forEach(function(bucket){
				console.trace(bucket.key);
				var value = (metric.type.name == 'count') ? bucket.doc_count : bucket[metric.id].value;
				data.push({y : value, color : colarr[ value % 10]});
			});
			console.trace(data);
			return {
				name: metric.params.customLabel,
				data: data
			}
		});
